...
ff = open("tree_doc.tsv",encoding="utf-8",mode="w")

to_compute_set = set()

def cos_distance_doc(doc1, doc2):
    global to_compute_set
    to_compute_set.add(doc1+"\t"+doc2)
    return 1
...
f3 = open("eval_mytree_center_.tsv",mode="w",encoding="utf-8")
...
ff = open("tree_doc_10.tsv",encoding="utf-8",mode="w")


for item_pair in to_compute_set:
    ff.write("-1\t")
    ff.write(item_pair)
    ff.write("\t-1\n")
