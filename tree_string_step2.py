...
f = open("question_pair.tsv",mode="r",encoding="utf-8")
lines1 = f.readlines()
f2 = open("test_results.tsv",mode="r",encoding="utf-8")
lines2 = f2.readlines()
questionpair2score = {}
for i in range(len(lines1)):
    splits = lines1[i].split("\t")
    questionpair2score[splits[1]+"||"+splits[2]]=lines2[i].rstrip().split("\t")[1]


cosine_count = 0
count = 0
f = open("all_questions.jsonl",mode="r",encoding="utf-8")
lines = f.readlines()
all_questions = []
for line in lines:
...
count = 0
f = open("all_embeddings.jsonl",mode="r",encoding="utf-8")
lines = f.readlines()
all_embeddings = []
for line in lines:
...
def cos_distance_doc(doc1, doc2):
    global questionpair2score
    return questionpair2score[doc1+"||"+doc2]
...
class Tree():
    ...
    def beam_search_doc(self, doc_string, query_vector, top_k=2):
        child_list = self.root_node.get_child_list()
        score_list = []
        for child in child_list:
            score_list.append(cos_distance_doc(all_questions[child.doc_id], doc_string))
...
def find_nearest_doc(center_node,childs):
    dis_list = []
    for c in childs:
        distance = cosine(center_node.vector, c.vector)
        dis_list.append(distance)
    sort = np.argsort(dis_list)
    return childs[sort[0]].doc_id
...
for i in range(cluster_num):
    ...
    for j in range(cluster_num):
        ...
        for k in range(cluster_num):
            ...
            docid = find_nearest_doc(node27,node27.get_child_list())
            node27.doc_id = docid

            node9.append_child(node27)

        docid = find_nearest_doc(node9, node9.get_child_list())
        node9.doc_id = docid

        node3.append_child(node9)

    docid = find_nearest_doc(node3, node3.get_child_list())
    node3.doc_id = docid
...
f3 = open("eval_mytree_center_doc_3.tsv",mode="w",encoding="utf-8")
...
for question in question2embedding:
    ...
    doc_ids = tree.beam_search_doc(question, emb, top_k=20)
        ...
        if (question != all_questions[idx]):
            if question in all_question_with_gold_label:
                ...
                if all_questions[idx] in true_pair and true_pair[all_questions[idx]] == question or \
                        all_questions[idx] in true_pair2 and true_pair2[all_questions[idx]] == question:
                    f3.write("1\n")
                else:
                    f3.write("0\n")
